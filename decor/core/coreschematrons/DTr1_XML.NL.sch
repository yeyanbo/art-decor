...
<pattern id="xml" xmlns="http://purl.oclc.org/dsdl/schematron">
    ...
    <rule context="*[count(ancestor::*)=0]">
        ...
        <assert role="error" test="not(contains($declaration,' version=')) or matches($declaration, 'version=.1\.0.')"
            >dtr1-1-XML: The XML prolog declaration SHALL show version 1.0.</assert>
        ...
        <assert role="error" test="not(contains($declaration,' encoding=')) or matches($declaration, 'encoding=.(utf-8)|(UTF-8).')"
            >dtr1-2-XML: The XML prolog declaration SHALL show encoding "UTF-8" or SHALL be empty.</assert>
        ...
        <assert role="error" test="/processing-instruction('nictiz') or not(contains($declaration,' standalone='))"
            >dtr1-3-XML: The XML prolog declaration SHALL not show standalone.</assert>
        ...
        <assert role="error" test="count(//processing-instruction()) = 0 or /processing-instruction('nictiz')"
            >dtr1-4-XML: The XML prolog declaration SHALL be the only processing instruction, e.g. no links to stylesheets are allowed.</assert>
    </rule>
    ...
    <rule context="*[@xsi:schemaLocation]">
        <assert role="warning" test="/processing-instruction('nictiz') or not(.)"
            >dtr1-5-XML: No links to XML Schemas are allowed. Found '<value-of select="@xsi:schemaLocation"/>'</assert>
    </rule>
    <rule context="*[@xsi:noNamespaceSchemaLocation]">
        <assert role="warning" test="/processing-instruction('nictiz') or not(.)"
            >dtr1-6-XML: No links to XML Schemas are allowed. Found '<value-of select="@xsi:noNamespaceSchemaLocation"/>'</assert>
    </rule>
    ...
    <rule context="*[@*[normalize-space()='']]">
        <assert role="error" test="not(.)">dtr1-8-XML: Attributes SHALL have a value</assert>
    </rule>
