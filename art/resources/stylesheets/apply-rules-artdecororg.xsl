...
-->
<xsl:stylesheet xmlns:f="http://orbeon.org/oxf/xml/formatting" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xi="http://www.w3.org/2001/XInclude" xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:atp="urn:nictiz.atp" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fr="http://orbeon.org/oxf/xml/form-runner" xmlns:xxforms="http://orbeon.org/oxf/xml/xforms" xmlns:widget="http://orbeon.org/oxf/xml/widget" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="2.0">
    <xsl:output method="xml"/>
    <xsl:strip-space elements="*"/>
   <!-- dummy parameter for 'touching' file to cause xinclude refresh -->
    <xsl:param name="lastRefresh" select="'2014-06-12T16:27:12.177+02:00'"/>
   ...
   <!-- variables for default logo and url -->
    <xsl:variable name="defaultLogo" select="'art-decor-logo40.png'"/>
    <xsl:variable name="defaultHref" select="'http://art-decor.org'"/>
   <!-- application menu  -->
   <!--    <xsl:variable name="applications" select="doc(concat($strArt-data,'/apps-list.xml'))" as="document-node()"/>-->
   <!--   <xsl:variable name="menu-template" select="doc('../art-menu-template.xml')" as="document-node()"/>-->
   <!-- menu template for art-decor.org -->
    <xsl:variable name="menu-template">
        <xi:include href="/db/apps/art/resources/artdecororg-menu-template.xml">
            <xi:fallback>
                ...
                <xhtml:table id="maincontent" width="100%" style="background: transparent;">
                   <!-- row with page heading and clickable logo -->
                    <xhtml:tr>
                        ...
                        <xhtml:td>
                            <xsl:variable name="logo" select="if (//xforms:instance[@id='logo']) then //xforms:instance[@id='logo']/logo else $defaultLogo"/>
                            <xsl:variable name="logoHref" select="if (//xforms:instance[@id='logo']) then //xforms:instance[@id='logo']/logo/@href else $defaultHref"/>
                        ...
                        <xhtml:td align="left" style="margin:0;padding:0;vertical-align:text-bottom;">
                            <xhtml:ul id="navmenu">
                                <xsl:for-each select="$menu-template/menu/section">
                                    <xsl:choose>
                                        <xsl:when test="$isDecor">
                                            <xsl:choose>
                                                <xsl:when test="@id='decor'">
                                                    <xhtml:li>
                                                        <xhtml:a href="{@link}">
                                                            <xforms:output ref="{concat('xxforms:instance(''menu'')/section[@id=''',@id,''']/name[@language=$resources/@xml:lang]')}"/>
                                                        </xhtml:a>
                                                        <xhtml:ul class="sub1">
                                                            <xforms:group model="art-menu" ref="instance('decor-menu')">
                                                                <xsl:for-each select="$decor-projects/projects/project[not(@repository='true')]">
                                                                    <xhtml:li>
                                                                        <xhtml:a href="{concat('decor-project--',@prefix)}">
                                                                            <xforms:output ref="replace(replace({concat('project[@prefix=''',@prefix,''']/name[@language=$resources/@xml:lang]')},'-','‑'),'\s+','&#160;')"/>
                                                                        </xhtml:a>
                                                                    </xhtml:li>
                                                                </xsl:for-each>
                                                                <xsl:for-each select="$decor-projects/projects/project[@repository='true']">
                                                                    <xhtml:li>
                                                                        <xhtml:a href="{concat('decor-project--',@prefix)}">
                                                                            <xsl:text>(BBR)&#160;</xsl:text>
                                                                            <xforms:output ref="replace(replace({concat('project[@prefix=''',@prefix,''']/name[@language=$resources/@xml:lang]')},'-','‑'),'\s+','&#160;')"/>
                                                                        </xhtml:a>
                                                                    </xhtml:li>
                                                                </xsl:for-each>
                                                ...
                                                <xsl:when test="@id=('project-overview','datasets','scenarios','project','decor-terminology','issues','decor-templates')">
                                                    <xhtml:li>
                                                        <xhtml:a href="{if (@link) then concat(@link,$document) else('#')}">
                                             <!--                                             <xsl:value-of select="name[@language='nl-NL']"/>-->
                                                            <xforms:output ref="{concat('xxforms:instance(''menu'')/section[@id=''',@id,''']/name[@language=$resources/@xml:lang]')}"/>
                                                        </xhtml:a>
                                                        <xsl:if test="application">
                                                            <xhtml:ul class="sub1">
                                                                <xsl:for-each select="application">
                                                                    <xhtml:li>
                                                                        <xhtml:a href="{concat(@link,$document)}">
                                                         <!--                                                         <xsl:value-of select="name[@language='nl-NL']"/>-->
                                        ...
                                        <xsl:otherwise>
                                            <xsl:choose>
                                             ...
                                             <!-- show menu items for available Snomed refsets -->
                                                ...
                                                <xsl:when test="@id=('demo','testing','tools','application')">
