...
let $codeSystemsAll         :=
    if (empty($projectPrefix)) then
        ...
        $get:colDecorData/decor[project/@prefix=$projectPrefix]/terminology/codeSystem[(@id|@ref)=$id]
    )
    
let $allAssociations        :=
    $decor/terminology/terminologyAssociation[@codeSystem=$codeSystemsAll/(@id|@ref)]
...
let $templatesVocabulary    := 
    $decor/rules/template//vocabulary[@codeSystem=$id]

(: Check if codesystem is the most recent version :)
let $mostRecent             := (empty($effectiveDate) or $effectiveDate=string(max($codeSystemsAll[@effectiveDate]/xs:dateTime(@effectiveDate))))
...
return
<usage prefix="{$projectPrefix}" id="{$id}" effectiveDate="{$effectiveDate}" mostRecent="{$mostRecent}">
{
    if ($mostRecent) then (
        for $concept in $inScopeConcepts[ancestor::decor/project/@prefix=$allAssociations/ancestor::decor/project/@prefix]
        ...
        let $dynBindings    := $allAssociations[@conceptId=$fullConcept/valueDomain/conceptList/(@id|@ref)]
        return
...
}
{
    ...
    let $inScope                := $codeSystemsAll[@codeSystem=$codeSystem]
    return
        if ($inScope) then (
