...
let $template               := $decor//template[@id=$templateAssociation/@templateId][@effectiveDate=$templateAssociation/@effectiveDate]| $decor//template[@ref=$templateAssociation/@templateId][@effectiveDate=$templateAssociation/@effectiveDate]
let $newAssociationInsert   :=
    ...
    <templateAssociation>
    {
        $templateAssociation/@*[not(name()='projectPrefix')],
        $templateAssociation/concept
    }
    </templateAssociation>
    </insert>
...
return
    if ($user=$decor/project/author/@username) then
        ...
        if ($association) then
        (
            update insert $templateAssociation/concept into $association,
