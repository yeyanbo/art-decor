...
<rule xmlns="http://purl.oclc.org/dsdl/schematron" abstract="true" id="EN">
    ...
    <assert role="error"
           test="@nullFlavor or (count(*)&gt;0 and string-length(normalize-space(string-join(text(),'')))=0) or (count(*)=0 and string-length(normalize-space(string-join(text(),'')))&gt;0)">dtr1-1-EN: entity name shall be null or text only or element only (no mixed content)</assert>
