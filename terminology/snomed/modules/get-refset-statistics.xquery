...
return
<refsetStatistics id="{$refsetId}" effectiveDate="{$refsetEffectiveDate}">
   {
   for $status in distinct-values($refset/member/@statusCode)
   return
   (
   <concepts statusCode="{$status}" count="{count($refset/member[@statusCode=$status])}"/>,   
   <translations statusCode="{$status}" count="{count($refset/member[@statusCode=$status][desc])}"/>
   )
   }
