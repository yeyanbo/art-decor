...
let $storedProject := collection(concat($get:strTerminologyData,'/snomed-extension/meta'))//project[@ref=$editedProject/@ref]
...
let $preparedProject :=
   <project>
      ...
      $editedProject/@*,
      $editedProject/name,
      for $desc in $editedProject/desc
      return
         art:parseNode($desc)
      ,
...
let $projectUpdate :=
   if ($storedProject/author[@username=$user][@admin='true']) then
       let $update := update replace $storedProject with $preparedProject
       return
       'OK'
   else('NO PERMISSION')
