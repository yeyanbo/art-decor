...
return
   if (string-length($collection) gt 0 and xmldb:collection-available(concat($get:strTerminologyData,'/snomed-extension/releases/',$collection))) then
   (
   response:set-header("Content-Disposition", concat('attachment; filename=',$collection,'.zip'))
   ,
   response:stream-binary(
       compression:zip( xs:anyURI(concat($get:strTerminologyData,'/snomed-extension/releases/',$collection,'/')), true() ),
       'application/zip',
