...
return
   if (string-length($collection) gt 0 and xmldb:collection-available(concat($get:strTerminologyData,'/dhd-data/releases/',$collection))) then
   (
   response:set-header("Content-Disposition", concat('attachment; filename=',$collection,'.zip'))
   ,
   response:stream-binary(
       compression:zip( xs:anyURI(concat($get:strTerminologyData,'/dhd-data/releases/',$collection,'/')), false() ),
       'application/zip',
