...
let $result := if ($validSearch) then 
								collection(concat($get:strTerminologyData,'/dhd-data/concepts'))//diagnose[ft:query(term,$query,$options)]
...
let $mappings := for $mapping in $result
					order by xs:integer($mapping/term[@type='referentie']/@count),xs:integer($mapping/term[@type='referentie']/@length)
					return
					$mapping
return
